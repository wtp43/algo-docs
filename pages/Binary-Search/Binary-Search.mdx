---
modified: 2024-12-31T02:54:49-05:00
---
# Binary Search

<CustomCallout type={"tip"} title={"tip"}>Can be used when `f(x)` is monotonically increasing/decreasing and the range for which `x` exists on is very large</CustomCallout>
...
<CustomCallout type={"intuition"} title={"intuition"}>Greedy: Greedily find the first strongest worker that can complete task[j]. If it isn't possible check if it is possible with pills.The best worker[i] to increase is not the weakest one or the strongest one but the one with smallest difference = task[j]-worker, for a worker that has not yet been assigned. Use bisect_left and SortedList with O(logn) removal from a list).https://leetcode.com/problems/maximum-number-of-tasks-you-can-assign/</CustomCallout>
