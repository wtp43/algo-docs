...
title: Bloom Filter
created: 2022-12-15
modified: 2024-12-30T22:51:33-05:00
---
...
<CustomCallout type="danger" title="danger">Useful for when knowing something is definitely not present or possibly present would be helpful. For instance, say we wanted to query a large database stored on a rotating hard drive (slow to read from). Before querying the disk, we could check for the record in a bloom filter and see if it's definitely not in there.</CustomCallout>
...
<CustomCallout type="Time-Complexity" title="Time Complexity">Look up and insert are O(1)</CustomCallout>

<CustomCallout type="Space-Complexity" title="Space Complexity">O(1)</CustomCallout>
