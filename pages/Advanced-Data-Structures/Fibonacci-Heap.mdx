...
created: 2022-12-13
modified: 2024-12-30T22:51:45-05:00
---
# Fibonacci Heap
...
1. A pointer is maintained at the minimum element node
1. It consists of a set of marked nodes (Decrease key operation)
1. The trees within a Fibonacci heap are unordered but rooted
...
1. Delete min node
1. Set the min-pointer to the next root in the root list
1. Create an array of size equal to the maximum degree of trees in the heap before deletion
1. Repeate (steps 5-7) until there are no multiple roots with the same degree
1. Map the degree of current root (min-pointer) to the degree in the array
1. Map the degree of next root to the degree in array
1. If there are more than two mappings for the same degree, then apply union operation to those roots such that the min-heap property is maintained

<CustomCallout type={"danger"} title={"danger"}>To minimize the heights of trees after unions, we have to merge in a way such that the degree of the last tree is at most "max degree". We should then merge trees with similar degrees which guarantees that the maximum degree will be the max degree pre-merge + 1</CustomCallout>
