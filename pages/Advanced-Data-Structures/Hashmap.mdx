...
title: Hashmaps
created: 2022-12-10
modified: 2024-12-30T23:34:38-05:00
---
# Hashmap
...
Consider the set of keys 𝐾=`{0,1,...,100}` and a hash table where the number of buckets is 𝑚=12m=12. Since 33 is a factor of 1212, the keys that are multiples of 33 will be hashed to buckets that are multiples of 33:

- Keys `{0,12,24,36,...}` will be hashed to bucket 00.
- Keys `{3,15,27,39,...}` will be hashed to bucket 33.
- Keys `{6,18,30,42,...}` will be hashed to bucket 66.
- Keys `{9,21,33,45,...}` will be hashed to bucket 99.
...
<CustomCallout type={"question"} title={"question"}>**How do you maintain O(1) insertion and lookup time complexity once the hashmap gets really full and linked list chains are long?**</CustomCallout>
>
