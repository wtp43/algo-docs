...
created: 2023-01-31
modified: 2024-12-30T22:18:57-05:00
---
## Dynamic Programming
...
<CustomCallout type={"note"} title={"note"}>Useful for subsequences or when there is no greedy solution. DP problems are essentially graph problems where the edges are not given to you.</CustomCallout>
...
<CustomCallout type={"tip"} title={"tip"}>Notice that since we are looking for the max sum and not the count of all partitions, we don't need to generate/keep track of all combinations</CustomCallout>
...
- For the $i^{th}$ bit, there are 3 options:
  ...
  1. It is included in $m$ but not in $s$
  1. Only in $s$
