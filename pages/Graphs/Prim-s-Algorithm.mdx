...
<CustomCallout type="example" title="example">```pythongraph = Graph(directed=False)graph.add_edge('A', 'B', 2)graph.add_edge('A', 'C', 3)graph.add_edge('B', 'C', 1)graph.add_edge('B', 'D', 1)graph.add_edge('B', 'E', 4)graph.add_edge('C', 'F', 5)graph.add_edge('D', 'E', 1)graph.add_edge('E', 'F', 1)graph.add_edge('F', 'G', 1)mst, total_cost = prim(graph, 'A')</CustomCallout>
>
