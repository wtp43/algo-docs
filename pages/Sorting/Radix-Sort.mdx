...
title: Radix Sort
created: 2022-12-15
modified: 2024-12-30T22:27:01-05:00
---
# Radix Sort
...
<CustomCallout type={"Time-Complexity"} title={"Time Complexity"}>O(n+k)</CustomCallout>

<CustomCallout type={"Space-Complexity"} title={"Space Complexity"}>O(max).If we take very large digit numbers or the number of other bases like 32-bit and 64-bit numbers then it can perform in linear time however the intermediate sort takes large space.This makes radix sort space inefficient. This is the reason why this sort is not used in software libraries.</CustomCallout>
