...
title: Bucket Sort
created: 2022-12-13
modified: 2024-12-30T22:24:50-05:00
---
...
<CustomCallout type={"danger"} title={"danger"}>Bucket sort/count sort is extremely useful (sorting in O(n) time) when you know the maximum number in the array. Then you can insert all items into a list of buckets then pop the items from the bucket sequentially.</CustomCallout>
...
<CustomCallout type={"Time-Complexity"} title={"Time Complexity"}>O($nlogn$), occurs if elements are all placed into the same buckets.Average case: O(n+k), occurs when elements are uniformly distributed and using \[[Insertion Sort]\]</CustomCallout>

<CustomCallout type={"Space-Complexity"} title={"Space Complexity"}>O(n+k) where n is the number of elements and k is the number of buckets formed.</CustomCallout>
